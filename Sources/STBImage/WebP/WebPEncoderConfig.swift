...
public struct WebPEncoderConfig: InternalRawRepresentable, Sendable {
    ...
    /// Hint for the image type (lossless only for now).
    public enum WebPImageHint: UInt32, Sendable {
        ...
        case photo = 2
        /// Discrete tone image (graph, map-tile, screenshot).
        case graph = 3
    ...
    /// In-loop filter strength, range [0 = off .. 100 = strongest].
    public var filterStrength: Int

    /// In-loop filter sharpness, range [0 = off .. 7 = least sharp].
    ...
    /// The minimum quality of the quantizer, in [0..100].
    public var qmin: Int = 0

    /// The maximum quality of the quantizer, in [0..100].
    ...
    /// Creates a configuration from one of libwebp's presets.
    ///
    /// - Parameters:
    ///   - preset: The preset to apply.
    ///   - quality: The quality, between 0 (smallest file) and 100 (biggest).
    /// - Returns: A validated-ready configuration with the preset's defaults.
    ...
    var rawValue: CWebP.WebPConfig {
        ...
        return CWebP.WebPConfig(
            ...
            method: Int32(method),
            image_hint: CWebP.WebPImageHint(rawValue: imageHint.rawValue),
